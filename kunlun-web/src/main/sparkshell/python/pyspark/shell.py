...
def create_rdd(nodes, indices, query):
  from pyspark import rdd
  from pyspark.rdd import RDD
  from pyspark.serializers import UTF8Deserializer

  conf = sc._jvm.java.util.HashMap()
  conf.put("es.nodes", nodes)
  conf.put("es.resource", indices)
  conf.put("es.query", query)
  conf.put("es.scroll.size", "5000")

  j_esData = sc._jvm.org.elasticsearch.spark.api.java.JavaEsSpark.esRDD(sc._jsc, conf)
  return RDD(j_esData, sc, UTF8Deserializer())
