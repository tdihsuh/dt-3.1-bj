define(['app'],function(app){
	app.controller('addConfDataSourceCtrl',['$scope','$http','$routeParams','$rootScope',function($scope,$http,$routeParams,$rootScope){
...
//				}
				...
				if(categoryValue == ""){
					...
					if(categoryNameValue!="" && typeValue != "" && datasourceNameValue !="" && ag.length<0){
						$scope.showData = 2;
			...
			if(wz==2){
				...
				if(protoValue == 'file' || protoValue == 'hdfs'){
					$scope.showLine = 1;
				}else{
					$scope.showLine = 2;
				}
...
//				  //alert(forwarderId); +"&forwarderId="+forwarderId
				var dataDo = "?categoryId="+datasource.categoryId+"&type="+datasource.type+"&agentId="+datasource.agentId;
					...
					dataDo +="&url="+datasource.url+"&encoding="+datasource.encoding;
