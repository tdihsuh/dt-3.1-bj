...
<table class="comm_table" cellspacing="0">
	...
	<c:forEach var="tRoleAuthority" items="${tRoleAuthorityList}" varStatus="ctIndex">
		...
		<c:if test="${ctIndex.count % 2 != 0}">
			...
			<td align="center" class="comm_td"><input type="checkbox" name="ids" value="<c:out value="${tRoleAuthority.id}"/>"/></td>
					<td class="comm_td">&nbsp;
							<c:out value="${tRoleAuthority.authorityId}"/>
					</td>
			<td align="center" class="comm_td">
				<a href="modify.hs?id=<c:out value="${tRoleAuthority.id}"/>">编辑</a>&nbsp;&nbsp;
				<a href="detail.hs?id=<c:out value="${tRoleAuthority.id}"/>">查看</a>&nbsp;&nbsp;
				<a href="javascript:" onclick="comm_del('<c:out value="${tRoleAuthority.id}"/>')">删除</a>
			</td>
		</tr>
	</c:forEach>
	<tr>
