            ...
            var mapFields = function(m) {
                ...
                _.each(m, function(types) {
                    _.each(types, function(type) {
                        fields = _.difference(_.union(fields, _.keys(type)), ['_parent', '_routing', '_size', '_ttl', '_all', '_uid', '_version', '_boost', '_source']);
                    });
                });
                return fields;
            };
                ...
                var request = ejs.client.get('/' + indices.join(',') + "/_mapping",
                    undefined, undefined, function(data, status) {
                        if (status === 0) {
                            ...
                            alertSrv.set('错误', "无法连接上 " + ejs.config.server + "/" +
                                indices.join(',') + "/_mapping。请确保至少有一个索引。" +
                                "如果您是通过代理连接，请确保代理正常。", 'error');
                        }
                    });
