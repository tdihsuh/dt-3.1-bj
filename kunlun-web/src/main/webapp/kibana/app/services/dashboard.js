        ...
        module.service('dashboard', function(
            ...
            $rootScope.$on('$routeChangeSuccess', function() {
                ...
                esVersion.isMinimum().then(function(isMinimum) {
                    ...
                    if (isMinimum) {
                        ...
                        alertSrv.set('请升级', "您使用的Elasticsearch版本太老，需要至少升级到" +
                            " Elasticsearch " + esMinVersion + " 或者更新的版本。", "error");
                    }
                });
            });
                ...
                if (self.current.index.interval !== 'none') {
                    ...
                    if (filterSrv.idsByType('time').length > 0) {
                        ...
                        kbnIndex.indices(_range.from, _range.to,
                            ...
                            if (p.length > 0) {
                                ...
                                if (self.current.failover) {
                                    ...
                                    alertSrv.set('无纪录', '您指定的时间段内查询不到任何纪录。', 'info', 5000);
                                    return false;
                                }
                            }
                        ...
                        if (self.current.failover) {
                            ...
                            alertSrv.set("无时间段",
                                '您的配置里，在时间段内无数据时不会切换成缺省索引，请配置好时间段。',
                                'info', 5000);
                        }
                    }
                } else {
            ...
            this.file_load = function(file) {
                return $http({
                    ...
                    alertSrv.set('错误', "无法加载控制台 <i>dashboards/" + file + "</i>。请确定它是否存在", 'error');
                    return false;
                });
            };
            ...
            this.elasticsearch_load = function(type, id) {
                ...
                var errorcb = function(data, status) {
                    if (status === 0) {
                        ...
                        alertSrv.set('错误', "无法找到 " + id + "。如果您是通过代理连接，请确保代理正常。", 'error');
                    }
                    return false;
                };
            ...
            this.script_load = function(file) {
                return $http({
                    ...
                    alertSrv.set('错误',
                        "无法打开 <i>scripts/" + file + "</i>。请确定它存在而且内容正确。",
                        'error');
                    return false;
                });
            };
            ...
            this.gist_list = function(id) {
                return $http.jsonp("https://api.github.com/gists/" + id + "?callback=JSON_CALLBACK").then(function(response) {
                    var files = [];
