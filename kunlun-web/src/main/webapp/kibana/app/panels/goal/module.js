        ...
        $scope.get_data = function() {
            ...
            results.then(function(results) {
                ...
                $scope.data = [{
                    label: 'Complete',
                    data: complete,
                    color: querySrv.colors[parseInt($scope.$id, 16) % 8]
                }, {
                    data: remaining,
                    color: Chromath.lighten(querySrv.colors[parseInt($scope.$id, 16) % 8], 0.70).toString()
                }];
                $scope.$emit('render');
            });
        };
            ...
            link: function(scope, elem) {
                ...
                elem.bind('plothover', function(event, pos, item) {
                    if (item) {
                        $tooltip
                            .html([
                                kbn.query_color_dot(item.series.color, 15), (item.series.label || ''),
