...
define([
    ...
    function(angular, app, _, kbn, moment) {
        ...
        var module = angular.module('kibana.panels.table_v2', []);
        app.useModule(module);

        module.controller('table_v2', function($rootScope, $http,$scope, $modal, $q, $compile, $timeout,
            ...
            $scope.panelMeta = {
                ...
                editorTabs: [{
                    title: '分页',
                    src: 'app/panels/table_v2/pagination.html'
                }, {
                    title: '查询',
                    src: 'app/partials/querySelect.html'
                }],
                status: "Stable",
                description: "多页显示查询结果详细纪录"
            };
            ...
            $scope.toggle_row_selection = function(row) {
                if (row.selected == true)
                    row.selected = false;
                else
                    row.selected = true;

                $scope.row_selection_changed(row.selected);
            }

            $scope.reset_rows_selection = function() {
                 _.each($scope.data, function(row) {
                    if (row.selected == true) row.selected = false;
                });                

                $scope.row_selection_changed(false);
            }

            $scope.row_selection_changed = function(rowSelected) {
                if (rowSelected == false) {
                    $scope.allRowsSelected = false;
                } else {
                    $scope.allRowsSelected = true;

                    var data = $scope.data.slice($scope.panel.offset, $scope.panel.offset+$scope.panel.size);

                     _.each(data, function(row) {
                        $scope.allRowsSelected &= row.selected;
                    });
                }
            }

            $scope.selDeselAll = function() {
                ...
                _.each(data, function(row) {
                    row.selected = $scope.allRowsSelected;
                });
            }

            $scope.submitWhitelists = function() {
                var whitelists = [];
                ...
                var isSqlIndection=false;
                if(whitelists.length>0){
                	
	                for(var i in whitelists){
	                	var category=whitelists[i]._source.category;
	                	var url=whitelists[i]._source.url;
	                	var index=whitelists[i]._index;
	                	var type=whitelists[i]._type;
	                	if(category!="MisConfiguration"){
	                		isSqlIndection=true;
	                	}else{
	                		$http.delete("../proxy/"+index+"/"+type+"/_query?q=url:\""+url+"\"").success(function(data){
	                			$http.post("../proxy/model/status_404/_search?q=url:\""+url+"\"").success(function(d){
									...
									if(d.hits.hits[0]){
										...
										$http.delete("../proxy/model/status_404/_query?q=url:\""+url+"\"").success(function(){
											$http.post("../proxy/manual/status_404/"+hashId+"/_create",para).success(function(){
												 //$scope.set_refresh(true);
												 dashboard.refresh();
											});
										}).error(function() {
						                    //alert("根据URL删除建模表失败！");
						                    console.log("根据URL删除建模表失败！");
						                });
									}else{
										...
										alert1=alert1+1;
										console.log("未找到匹配的模型数据！");
									}
								}).error(function() {
				                    //alert("根据URL查询建模表失败！");
				                    console.log("根据URL查询建模表失败！");
				                });
	                		});
	                	}
	                }
