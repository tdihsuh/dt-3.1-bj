...
define([
    ...
    function(angular, app, _, $, kbn) {
        ...
        var module = angular.module('kibana.panels.terms_v2', []);
        app.useModule(module);

        module.controller('terms_v2', function($scope, querySrv, dashboard, filterSrv, fields) {
            $scope.panelMeta = {
                modals: [{
                    description: "查看",
                    icon: "icon-info-sign",
            ...
            $scope.build_mterms_search = function() {
                var terms = $scope.panel.selectedTerms;
                ...
                var r = filterSrv.set({
                    ...
                    value: terms,
                    mandate: 'must'
                }, $scope.filterID, true, true);

                $scope.filterID = (r !== false)? r : undefined;
                dashboard.refresh();
            };
            
            $scope.reset_mterms_search = function() {

                var r = filterSrv.remove($scope.filterID, true);
                $scope.panel.selectedTerms = [];
            ...
            // Check if the associated filter has been removed from the fitlers panel
            $scope.$on('filter', function() {
                if (!_.isUndefined($scope.filterID) && _.indexOf(filterSrv.ids(), $scope.filterID) == -1) {
            ...
            return {
                ...
                link: function(scope, elem) {
                    ...
                    function build_results() {
                        ...
                        var convertIP = (scope.field.indexOf("_ip") >= 0);

                        _.each(scope.results.facets.terms.terms, function(v) {
                            var slice;
                            if (convertIP) v.term = num2dot(v.term);
                            //scope.panel.selectedTerms.push(v.term);
                    ...
                    function render_panel() {
                        ...
                        chartData = scope.panel.missing ? chartData :
                            _.without(chartData, _.findWhere(chartData, {
                                meta: 'missing'
                            }));
