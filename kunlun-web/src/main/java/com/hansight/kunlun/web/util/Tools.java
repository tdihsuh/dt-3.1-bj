...
public class Tools {
     ...
     * @return
     */
	public static boolean compareUrl(String requestUrl,String url){
    	if(url.equals("/*")) return true;
    	
    	//如果请请求地址中含参数（?），则截取参数（?）之前的地址
    	...
    	if(url.contains("/*")){
    		...
    		if(!urls[0].equals("")){
    			//"*"号前路径不一致返回false
    			if(requestUrl.indexOf(urls[0]) == 0){
    				return true;
    			}
    		}
    		return false;
    		...
    		//验证不含"*"号的地址
    		return compareUrlMethod(requestUrl,url);
    	}
    }
     ...
     * @return
     */
    public static boolean compareUrlMethod(String requestUrl,String url){
    	if(url.equals("/*")) return true;
    	...
    	if(url.contains("/*")){
    		String[] urls = url.split("/\\*");
    		String preRequestUrl = requestUrl.substring(0,requestUrl.lastIndexOf("/"))+"/";
    		if(preRequestUrl.endsWith(urls[0]+"/") && (url.endsWith("/*") || requestUrl.endsWith(urls[1])))
    			return true;
    	}
    ...
    public static void main(String[] args){
    	String requestUrl = "/#/usermng";
    	String url = "/#/*";
    	System.out.println(Tools.compareUrl(requestUrl,url));
    }
}
